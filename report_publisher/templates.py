...
def performance_report(server_name, root_path, output_dir, working_folder_path, model_name, labels, metric_charts, log, total_count, FN_links, FN_img_links, failed_expected_labels, failed_expected_counters, failed_detected_labels, failed_detected_counters, report, DT):
...
def focused_performance_report(focus_labels, server_name, root_path, output_dir, working_folder_path, model_name, labels, metric_charts, log, total_count, FN_links, failed_expected_labels, failed_expected_counters, failed_detected_labels, failed_detected_counters, report, DT):
            ...
            if len(FN_links) != 0:
                ...
                for l in range(len(failed_expected_labels)):
                    if failed_expected_labels[l] in focus_labels:
                        ...
                        if errors == 0:
                            with tag('span', klass="description"):
                                text('No errors found for expected {} with threshold >= {}%'.format(failed_expected_labels[l], DT))
                if len(focus_labels) > 2:
                    errors = 0
                    for l in range(len(failed_detected_labels)):
                        ...
                        if errors == 0:
                            with tag('span', klass="description"):
                                text('No errors found for detected {} with threshold >= {}%'.format(failed_detected_labels[l], DT))                               
            # Confusion matrix and ROC AUC sections
            with tag('h2', klass="new-page"):
                text('2 - Confusion Matrix')
            with tag('p'):
...
def summary_performance_report(focus_labels, server_name, root_path, output_dir, working_folder_path, model_name, labels, metric_charts, log, total_count, FN_links, failed_expected_labels, failed_expected_counters, failed_detected_labels, failed_detected_counters, report, DT):
